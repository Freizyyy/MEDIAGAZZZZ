...
const Navbar: React.FC<NavbarProps> = ({ activeTab, setActiveTab }) => {
  const navItems = [
    ...
    { id: AppTab.FEEDBACK, label: 'Связь', icon: <Icons.Mail /> },
  ];

  return (
    <>
      {/* Top Bar for Logo - Persistent on Desktop, Simplified on Mobile */}
      <nav className="fixed top-0 left-0 right-0 z-[100] px-4 pt-4 sm:pt-6 pointer-events-none">
        <div className="max-w-7xl mx-auto glass rounded-[2rem] sm:rounded-[3rem] h-16 sm:h-24 px-6 sm:px-10 flex items-center justify-between shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] pointer-events-auto border border-white/50 backdrop-blur-3xl">
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center bg-gray-100/50 p-2 rounded-[2rem] gap-2 border border-white shadow-inner">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => setActiveTab(item.id)}
                className={`flex items-center space-x-3 px-8 py-4 rounded-2xl transition-all duration-500 relative overflow-hidden group/btn ${
                  activeTab === item.id 
      ...
      {/* Mobile Bottom Navigation - Optimal for one-handed use */}
      <div className="lg:hidden fixed bottom-6 left-4 right-4 z-[100] h-16 glass rounded-2xl border border-white/40 shadow-2xl flex items-center justify-around px-2 pointer-events-auto">
        {navItems.map((item) => (
          ...
          >
            ...
            <span className={`text-[10px] font-bold uppercase tracking-wider transition-opacity ${activeTab === item.id ? 'opacity-100' : 'opacity-60'}`}>
              {item.label}
            </span>
            {activeTab === item.id && (
